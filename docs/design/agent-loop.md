...
IAgentPipeline
  ...
  8000 RunIterationMiddleware           before: IAgentIterationRunner.RunAsync → context.Outcome; then next (no-op terminal)
```
...
- **Lock / interrupt CTS / activity / correlation / tool re-enqueue / system prompt / iteration.** Middleware.
