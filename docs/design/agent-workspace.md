...
| `system/DEFAULT.md`, `system/MINIMAL.md`, `system/SUBAGENT.md`, `system/COMPACTION.md` | Rendered (Scriban) and used as the agent's system prompt. Selection: `AgentConfig.SystemPrompt` (defaults to `DEFAULT`). The compaction child overlays `COMPACTION.md`. Fallback chain: workspace `<name>.md` → workspace `DEFAULT.md` → bundled `<name>.md` → bundled `DEFAULT.md`. | [`FilesystemSystemPromptProvider`](../../src/LlamaShears.Core/SystemPrompt/FilesystemSystemPromptProvider.cs) |
| `system/context/PROMPT.md`, `system/context/SUBAGENT.md`, `system/context/COMPACTION.md` | Renders the per-turn ephemeral block. Same fallback chain as the system prompt. Selection: `AgentConfig.PromptContext` (defaults to `PROMPT`; `subagent_run` overlays `SUBAGENT`; the compaction child overlays `COMPACTION`). | [`FilesystemPromptContextProvider`](../../src/LlamaShears.Core/PromptContext/FilesystemPromptContextProvider.cs) |
