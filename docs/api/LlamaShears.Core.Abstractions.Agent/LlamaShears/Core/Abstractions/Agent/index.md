...
- [IAgentFactory](IAgentFactory.md) — Surfaces the catalog of agents a host knows about and constructs [IAgent](IAgent.md) instances from [AgentConfiguration](AgentConfiguration.md). Implementations decide where the catalog comes from (disk, registry, in-memory) and what construction means (DI activation, plugin resolution).
