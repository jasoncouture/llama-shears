...
### `Issue`([AgentInfo](AgentInfo.md) agent)

Issue a fresh token bound to `agent`. The token is a
base64-encoded opaque string. Callers must treat it as a credential.

### `TryGetAgentInformation`(string token, [AgentInfo](AgentInfo.md)& agent)

Atomically consume `token`: if it is a known and
unexpired token, return its bound [AgentInfo](AgentInfo.md) and remove
