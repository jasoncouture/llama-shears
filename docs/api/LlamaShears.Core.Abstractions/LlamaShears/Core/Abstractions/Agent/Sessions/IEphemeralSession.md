# LlamaShears.Core.Abstractions.Agent.Sessions.IEphemeralSession
...
`<agentId>/<sessionGuid>/current.json`, and emits a
...
pair under which its transcript persists. `SessionId` is
never null on an ephemeral session.

## Methods

### `RunAsync`(string initialPrompt, CancellationToken cancellationToken)

Drives the session's private iteration loop against
`initialPrompt`, persisting all turns and tool
...
is reached, or `cancellationToken` trips.
