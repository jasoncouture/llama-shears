...
### `EnqueueAsync`([ModelTurn](../../Provider/ModelTurn.md) turn, CancellationToken cancellationToken)

Appends `turn` to the appropriate internal lane.
User turns batch by [ModelTurn](../../Provider/ModelTurn.md).`ChannelId`; tool turns
drain ahead of any pending user batch on the next dequeue.

#### Parameters

- `turn` — The turn to queue. Must have `Role` set to [ModelRole](../../Provider/ModelRole.md).`User` or [ModelRole](../../Provider/ModelRole.md).`Tool`.
