...
- [IAgentIterationRunner](IAgentIterationRunner.md) — Runs a single agent iteration from the pipeline bag: builds the prompt (including [AgentPipelineContext](Pipeline/AgentPipelineContext.md).`SystemPrompt` and [AgentPipelineContext](Pipeline/AgentPipelineContext.md).`EphemeralContext`), invokes the language model (with the empty-response retry), persists the model's output via the active context store, and returns any tool-result turns the caller should feed back on the next iteration. Compaction belongs to the surrounding onion, after this call returns. Knows nothing about session queues, agent locks, or interrupt subscriptions.
