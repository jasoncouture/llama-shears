...
- [SubagentRunResult](SubagentRunResult.md) — Outcome of [ISubagentRunner](ISubagentRunner.md).`RunAsync`. [SubagentRunResult](SubagentRunResult.md).`Ok` means the child finished while the caller waited. [SubagentRunResult](SubagentRunResult.md).`Error` is refuse / spawn / timeout transport — not the child's HTTP-style status.
