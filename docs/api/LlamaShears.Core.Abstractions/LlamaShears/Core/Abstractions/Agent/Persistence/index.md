...
- [ArchiveId](ArchiveId.md) — Identifies a single archived (rotated-out) context file. The tuple ([ArchiveId](ArchiveId.md).`AgentId`, [ArchiveId](ArchiveId.md).`SessionId`, [ArchiveId](ArchiveId.md).`UnixMillis`) uniquely names `<AgentId>/<UnixMillis>.json` (default session, when [ArchiveId](ArchiveId.md).`SessionId` is Guid.`Empty`) or `<AgentId>/<sessionGuid>/<UnixMillis>.json` (non-default session) in the context store.
