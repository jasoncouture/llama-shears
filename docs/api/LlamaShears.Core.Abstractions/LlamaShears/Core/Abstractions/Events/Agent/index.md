...
- [AgentCompactionMarker](AgentCompactionMarker.md) — Empty payload for Agent.`CompactingStarted` / Agent.`CompactingFinished`. Carries no data — its presence on the bus is the signal.
...
- [AgentToolCallFragment](AgentToolCallFragment.md) — Event-bus payload describing a single tool call the agent is about to dispatch. Mirrors [ToolCall](../../Provider/ToolCall.md); kept distinct so consumers of the event bus don't have to depend on the provider layer.
