...
- [AgentToolCallFragment](AgentToolCallFragment.md) — Event-bus payload describing a single tool call the agent is about to dispatch. Mirrors Provider.`ToolCall`; kept distinct so consumers of the event bus don't have to depend on the provider layer.
