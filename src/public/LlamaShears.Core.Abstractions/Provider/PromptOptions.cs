...
/// <param name="SystemPromptTemplate">File name (including extension) of the system-prompt template the inference runner should render and prepend to the prompt; <see langword="null"/> skips the system turn so callers like the compactor can manage their own.</param>
public sealed record PromptOptions(
    ...
    string? SystemPromptTemplate = PromptOptions.DefaultSystemPromptTemplate
)
{
    private const string DefaultSystemPromptTemplate = "DEFAULT.md";
}
