...
description: Public onion around each dequeued batch; IAgentMiddleware.Order (built-ins 1000 apart plus ToolLoopLimit 10500) is outer-to-inner; IActiveTurnCancellation / IAgentLifetime seams; inference runner is a later cleanup
type: project
---
...
Per-batch work is a public onion under `LlamaShears.Core.Abstractions.Agent.Pipeline`. `IAgentMiddleware.Order` is **outer-to-inner** (lowest outermost). Built-ins use `AgentMiddlewareOrder` spaced 1000: TurnException 1000, AgentActivity 2000, CorrelationScope 3000, AgentLock 4000, InterruptScope 5000, ToolResultEnqueue 6000, Compaction 7000, SystemPrompt 8000, EphemeralContext 9000, RunIteration 10000, ToolDispatch 11000, StripImageAttachments 12000. `ToolLoopLimit` 10500 sits in the RunIteration/ToolDispatch gap and drops leftover tool calls when `IToolLoopBudget.IsFinal` (`Tools.TurnLimit`, 0 = unlimited). Plugins pick a value in a gap (or `< 1000` / `> 12000`). Equal orders keep enumeration order. `AddAgentMiddleware<T>()` is `TryAddEnumerable(Scoped<IAgentMiddleware, T>)` plus idempotent `TryAddScoped<IAgentPipeline, AgentPipeline>` — registration sequence is not the fold. The terminal is a no-op; the innermost step must do the work. `CompactionMiddleware` publishes the inbound batch, optionally overlays `AgentConfig.SystemPrompt` **and** `PromptContext` to `COMPACTION.md`, `next()`s a summarizer pass, restores both names, then sets `Prompt` to the live turns or `[summary + last 6]`. It does not render those templates. `SystemPromptMiddleware` writes `SystemPrompt` and prepends it onto `Prompt`. `EphemeralContextMiddleware` stamps `IAgentStateTracker`, writes `EphemeralContext` (memory search + `config.PromptContext`), and inserts that turn into `Prompt` once before the last user cluster when the prompt ends in a user turn (skips if a `SystemEphemeral` turn is already present). A rendered template with `Prompt` still null throws. `IAgentIterationRunner` requires `Prompt` and `SessionId` and sends the prompt as-is (`EmitTurns: !FrameworkPass`). `RunIterationMiddleware` copies `SessionId` from the data scope and `ChannelId` from the inbound batch onto the bag, then strips image attachments from live context in a `finally` unless `FrameworkPass`. `IInferenceRunner.RunAsync` takes `sessionId`, `correlationId`, and `channelId` as arguments, does not read the prompt or the data scope, and does not dispatch tools. `ToolDispatchMiddleware` executes `Outcome.ToolCalls` via `ToolCallExecutor` and writes `ToolResultTurns`. `StripImageAttachmentsMiddleware` drops leftover images after tools unless `FrameworkPass`. Image attachments are never written to the store; see `project_image_attachments_ephemeral.md`. See `project_compaction.md`.
