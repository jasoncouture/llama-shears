...
Per-batch work is a public onion under `LlamaShears.Core.Abstractions.Agent.Pipeline`. `IAgentMiddleware.Order` is **outer-to-inner** (lowest outermost). Built-ins use `AgentMiddlewareOrder` spaced 1000: TurnException 1000, AgentActivity 2000, CorrelationScope 3000, AgentLock 4000, InterruptScope 5000, ToolResultEnqueue 6000, Compaction 7000, SystemPrompt 8000, EphemeralContext 9000, RunIteration 10000, ToolDispatch 11000, StripImageAttachments 12000. `ToolLoopLimit` 10500 sits in the RunIteration/ToolDispatch gap and drops leftover tool calls when `IToolLoopBudget.IsFinal` (`Tools.TurnLimit`, 0 = unlimited). Plugins pick a value in a gap (or `< 1000` / `> 12000`). Equal orders keep enumeration order. `AddAgentMiddleware<T>()` is `TryAddEnumerable(Scoped<IAgentMiddleware, T>)` plus idempotent `TryAddScoped<IAgentPipeline, AgentPipeline>` — registration sequence is not the fold. The terminal is a no-op; the innermost step must do the work. `CompactionMiddleware` publishes the inbound batch, optionally awaits `ISubagentRunner` (`COMPACTION.md` on both templates, transcript as the prompt), commits `[summary + last 6]`, then `next()`s once. It does not overlay the parent config. `SystemPromptMiddleware` writes `SystemPrompt` and prepends it onto `Prompt`. `EphemeralContextMiddleware` stamps `IAgentStateTracker`, writes `EphemeralContext` (memory search + `config.PromptContext`), and inserts that turn into `Prompt` once before the last user cluster when the prompt ends in a user turn (skips if a `SystemEphemeral` turn is already present). A rendered template with `Prompt` still null throws. `IAgentIterationRunner` requires `Prompt` and `SessionId` and sends the prompt as-is (`EmitTurns: true`). `RunIterationMiddleware` copies `SessionId` from the data scope and `ChannelId` from the inbound batch onto the bag, then strips image attachments from live context in a `finally`. `IInferenceRunner.RunAsync` takes `sessionId`, `correlationId`, and `channelId` as arguments, does not read the prompt or the data scope, and does not dispatch tools. `ToolDispatchMiddleware` executes `Outcome.ToolCalls` via `ToolCallExecutor` (`Task.WhenAll`, cap 15) and writes `ToolResultTurns` in original call order. `StripImageAttachmentsMiddleware` drops leftover images after tools. Image attachments are never written to the store; see `project_image_attachments_ephemeral.md`. See `project_compaction.md`.
