...
- `saveAs` writes the full body into the workspace — same write confinement as `file_write` (`WorkspacePathResolver`, `system/` banned, protection policy). 16 MiB cap (`savedTruncated`). Existing file requires `overwrite=true`. This is how agents fetch images/PDFs/binaries.
